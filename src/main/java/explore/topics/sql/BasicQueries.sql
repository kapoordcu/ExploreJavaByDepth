...
-- // Select 2nd Max salary in Employee table
...
select e.FNAME, d.DNAME, e.SALARY
...
from EMPLOYEE
ORDER BY SALARY DESC LIMIT 3) RESULT ORDER BY SALARY LIMIT 1;

-- // Using CTE = Common Table Expression (DUPLICATE CREATES A PROBLEM) // Dont use ROW_NUMBER
WITH RESULT AS (SELECT SALARY, DENSE_RANK() OVER (Order By SALARY DESC ) DENSRANK
FROM EMPLOYEE)
Select SALARY FROM RESULT where DENSRANK=2;
